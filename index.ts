...
import { FnContext, FnEvent, FnHandlerFile, FnResult } from '@snapcore/snapcore-types'
...
import { handler, init, shutdown, ready } from './function/handler'
...
class FunctionEvent implements FnEvent {
    ...
    constructor(req: Request) {
        ...
        this.path = req.path
        if (!this.requestContext) this.requestContext = { requestId: Ulid.generate().toRaw() }
        else this.requestContext.requestId = Ulid.generate().toRaw()
    }
}
