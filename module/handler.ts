...
export async function handler(event: FnEvent, context: FnContext, callback?: FnCallback): Promise<FnResult> {
    const result: FnResult = {
        statusCode: 200,
        body: {
            status: 'Hello world from OpenFaas: ' + JSON.stringify(event.body)
        }
    }
...
export function ready(context: FnContext) {
    console.log(`OpenFaaS Node.js listening on http://localhost:${context.port} in ${context.environment}`)
}
